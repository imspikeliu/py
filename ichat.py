...
while 1:  # 无限循环
    ...
    if now_str in ['15:00:00']:
        itchat.send('There are two motives for reading a book: one, that you enjoy it; the other, that you can boast about it.', toUserName=userName)
    if now_str in ['16:00:00']:
        itchat.send('As selfishness and complaint pervert the mind,so love with its joy clears and sharpens the vision.', toUserName=userName)
    if now_str in ['17:00:00']:
        itchat.send('快要下班啦，工作做完了吗？做完了就看点资料学习下吧，放松放松！', toUserName=userName)
    if now_str in ['17:30:00']:
        itchat.send('下班啦，注意安全，走路别玩手机！！！', toUserName=userName)
